...
if(len(sys.argv) < 3):
	sys.exit("usage: python[3] simSerial_noRandom.py [num lattice points each side of square] [num timesteps]") 
	time.sleep(1)
...
class Lattice:
	...
	#static counter (removes random functionality)
	counter = 0
	...
	def stageNextState(self, index, size):
		...
		#50% chance of resting on top
		#50% chance of saturating earth
		elif self.state == 2:
			if Lattice.counter % 2 == 1:
				self.future = 2
			else:
				self.future = 1
				Lattice.counter += 1
